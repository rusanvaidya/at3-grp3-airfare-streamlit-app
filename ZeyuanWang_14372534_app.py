...
if page == "Home":
    ...
    if st.button("Predict Fare"):
        ...
        if origin_code in ["ATL", "MIA", "CLT", "DFW"]:
            # Map the cabin type only for Zeyaun Wang's model
            cabin_type_mapped = map_cabin_type(cabin_type)
            
            model, label_encoders, feature_scaler, target_scaler = load_zeyuan_model()
            input_data_scaled = preprocess_input_for_zeyuan(origin_code, destination_code, departure_date, departure_time, cabin_type_mapped, label_encoders, feature_scaler)
